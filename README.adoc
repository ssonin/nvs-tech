...
* *State columns*: `state` exists on clients and documents for future soft-delete support, but current queries do not filter on it
* *Application-enforced document ownership*: client existence is checked before document insert, but `documents.client_id` is not currently enforced by a database foreign key
...
Some tests also use Kafka Testcontainers. A working Docker environment is required for the full suite.
...
Note that `/api/v1/search` depends on the embedding service for every request. If the embedding service is down, search currently returns `500`, while document creation can still succeed because embeddings are populated asynchronously.
...
[source,bash]
----
curl http://localhost:8083/connectors/searchapi-postgres/status
----
